...
//#endregion
//#region frontend/advisor/src/i18n.js
...
function getPathValue(obj, key) {
	return String(key || "").split(".").reduce((acc, part) => acc && typeof acc === "object" ? acc[part] : void 0, obj);
}
...
async function initI18n() {
	...
	const browser = Array.isArray(navigator.languages) && navigator.languages.length ? navigator.languages[0] : navigator.language;
...
function t(key, vars = {}) {
	...
	return interpolate(getPathValue(current, key) ?? getPathValue(fallback, key) ?? key, vars);
}
...
//#endregion
//#region frontend/advisor/src/app.js
var SAMPLE_PORTFOLIO_CSV_URL = "/static/sample_portfolio.csv";
var PORTFOLIO_CSV_COLUMNS = [
	"portfolio_name",
	"benchmark",
	"cash_usd",
	"review_date",
...
function todayLocalIso() {
	...
	const offsetMs = now.getTimezoneOffset() * 60 * 1e3;
...
var lastStartBody = null;
var currentGlobalStatusState = "idle";
var currentDataLoaderUi = {
	state: "idle",
...
var AGENT_PLANS = {
	planner: {
		...
		descKey: "agents.planner.desc",
		stepKeys: [
			"agents.planner.steps.0",
			"agents.planner.steps.1",
			"agents.planner.steps.2"
		]
	},
	data: {
		labelKey: "agents.data.label",
		descKey: "agents.data.desc",
		stepKeys: [
			"agents.data.steps.0",
			"agents.data.steps.1",
			"agents.data.steps.2",
	...
	risk: {
		labelKey: "agents.risk.label",
		descKey: "agents.risk.desc",
		stepKeys: [
			"agents.risk.steps.0",
			"agents.risk.steps.1",
			"agents.risk.steps.2",
	...
	regime: {
		labelKey: "agents.regime.label",
		descKey: "agents.regime.desc",
		stepKeys: [
			"agents.regime.steps.0",
			"agents.regime.steps.1",
			"agents.regime.steps.2",
			"agents.regime.steps.3",
			"agents.regime.steps.4"
		]
	},
	theme: {
		labelKey: "agents.theme.label",
		descKey: "agents.theme.desc",
		stepKeys: [
			"agents.theme.steps.0",
			"agents.theme.steps.1",
			"agents.theme.steps.2"
		]
	},
	validation: {
	...
	manager: {
		...
		descKey: "agents.manager.desc",
		stepKeys: [
			"agents.manager.steps.0",
			"agents.manager.steps.1",
			"agents.manager.steps.2"
		]
	}
};
var _drawerOpen = false;
var _drawerAgent = null;
var _drawerPinned = false;
var _autoFollowTimer = null;
var AUTO_FOLLOW_DELAY_MS = 3e3;
var _agentOutputs = {};
var agentSummaryQueue = [];
var activeAgentSummary = null;
var pendingFinalResult = null;
...
var dataLoaderHistory = [];
var dataLoaderTickerStatus = /* @__PURE__ */ new Map();
var PRIORITY_ALIASES = new Map([
	["urgent", "urgent"],
	["immediate", "urgent"],
	["critical", "urgent"],
	["high", "urgent"],
...
function sidebarWidthMax() {
	return Math.min(Math.floor(window.innerWidth * .5), SIDEBAR_MAX_PX);
}
...
function initSidebarResize() {
	...
	window.addEventListener("resize", () => {
		const cur = parseFloat(getComputedStyle(document.documentElement).getPropertyValue("--sidebar-width").trim() || "260");
...
var _reviewEndTime = null;
var _reviewTimerId = null;
...
function fmtDuration(totalSecs) {
	...
	if (s > 0) return t("common.durationMinutesSeconds", {
		minutes: m,
		seconds: s
	});
...
function reviewElapsedSeconds(nowMs = Date.now()) {
	...
	return Math.max(0, Math.floor(((_reviewEndTime || nowMs) - _reviewStartTime) / 1e3));
}
function renderReviewElapsed() {
	const el = document.getElementById("review-elapsed-value");
	if (!el) return;
	el.textContent = _reviewStartTime ? fmtDuration(reviewElapsedSeconds()) : t("sidebar.timeNotStarted");
}
...
function mergeAgentOutput(existing, incoming) {
	if (existing && typeof existing === "object" && !Array.isArray(existing) && incoming && typeof incoming === "object" && !Array.isArray(incoming)) return {
		...existing,
		...incoming
	};
	return incoming;
...
async function syncReviewSnapshot(reviewId, options = {}) {
	...
	try {
		...
		if (!silent) console.warn("[analysis trace snapshot sync failed]", err);
		return null;
	}
}
...
function currentDataLoaderDetailInput() {
	if (currentDataLoaderUi.detailKey) return dataLoaderDetail(currentDataLoaderUi.detailKey, currentDataLoaderUi.detailVars);
	return currentDataLoaderUi.detailText;
}
function resolveDataLoaderDetail(detail) {
	if (detail && typeof detail === "object" && !Array.isArray(detail) && detail.key) return t(detail.key, detail.vars || {});
	return String(detail || "").trim();
}
function normalizeDataLoaderDetail(detail) {
	if (detail && typeof detail === "object" && !Array.isArray(detail) && detail.key) return {
...
function recordDataLoaderEvent(state, detail) {
	...
	if (last && last.state === state && last.detail === text) return;
	...
	if (dataLoaderHistory.length > DATA_LOADER_HISTORY_MAX) dataLoaderHistory.length = DATA_LOADER_HISTORY_MAX;
...
function initializeDataLoaderTickerStatus(positions) {
	...
	for (const ticker of normalizeTickerList((positions || []).map((p) => p?.ticker))) dataLoaderTickerStatus.set(ticker, "queued");
	renderDataLoaderHistory();
}
function setAllDataLoaderTickers(state) {
	for (const ticker of dataLoaderTickerStatus.keys()) dataLoaderTickerStatus.set(ticker, state);
	renderDataLoaderHistory();
}
function setDataLoaderTickers(state, tickers) {
	for (const ticker of normalizeTickerList(tickers)) if (dataLoaderTickerStatus.has(ticker)) dataLoaderTickerStatus.set(ticker, state);
	renderDataLoaderHistory();
}
...
function dataLoaderTickerStatusClass(state) {
	return `data-loader-ticker-state data-loader-ticker-state-${[
		"queued",
		"loading",
		"loaded",
		"error"
	].includes(state) ? state : "queued"}`;
}
function renderDataLoaderHistory() {
	...
	list.innerHTML = Array.from(dataLoaderTickerStatus.entries()).map(([ticker, state]) => `
      <li>
...
function toggleDataLoaderExpanded() {
	const toggle = document.getElementById("card-data");
	if (!toggle) return;
	setDataLoaderExpanded(!(toggle.getAttribute("aria-expanded") === "true"));
}
function setDataLoaderStatus(state, detail = "", allowRetry = false, options = {}) {
	...
	currentDataLoaderUi = {
		state,
		...normalizeDataLoaderDetail(detail),
		allowRetry
	};
	...
	if (badge) {
		...
		badge.title = text;
		if (!badge.classList.contains("agent-status-label")) badge.className = `badge badge-${state}`;
	}
	...
	if (recordHistory) recordDataLoaderEvent(state, text);
}
...
function formatDataLoaderError(message) {
	...
	if (analogMissing) return t("dataLoader.missingAnalogHistory", { items: analogMissing[1] });
	const holdingsMissing = text.match(/missing price history for holdings:\s*(.+)$/i);
	if (holdingsMissing) return t("dataLoader.missingHoldingsHistory", { items: holdingsMissing[1] });
...
function getPositionMarketValue(wrap) {
	...
	return !Number.isNaN(q) && q !== 0 && !Number.isNaN(cur) ? q * cur : 0;
}
function refreshMetrics(wrap) {
	...
	if (valEl) {
		...
		if (qtyOk && curOk) valEl.textContent = fmtUsd(q * cur);
		else {
			valEl.textContent = "—";
...
async function refreshAllQuotes() {
	...
	if (!wraps.some((w) => w.querySelector("[data-field=\"ticker\"]")?.value?.trim())) {
...
function positionRowsMissingReadyQuotes() {
	return positionRowsMissingReadyQuoteElements().map((wrap) => wrap.querySelector("[data-field=\"ticker\"]")?.value?.trim()?.toUpperCase()).filter(Boolean);
}
function positionRowsMissingReadyQuoteElements() {
	return Array.from(document.querySelectorAll("#positions-body .position-row-wrap")).filter((wrap) => {
		if (!wrap.querySelector("[data-field=\"ticker\"]")?.value?.trim()) return false;
...
function refreshPositionsState() {
	...
	const noteCount = rows.filter((wrap) => wrap.querySelector("[data-field=\"entry_thesis\"]")?.value?.trim()).length;
	const meta = document.getElementById("positions-meta");
	if (meta) if (total === 0) meta.textContent = t("positions.metaNone");
	else meta.textContent = t("positions.metaLoaded", {
		count: total,
		noteCount,
		positionLabel: total === 1 ? t("positions.positionSingular") : t("positions.positionPlural"),
		noteLabel: noteCount === 1 ? t("positions.noteSingular") : t("positions.notePlural")
	});
	...
	if (refreshBtn && refreshBtn.getAttribute("aria-busy") !== "true") refreshBtn.disabled = total === 0;
}
function updateWeightSummary() {
	const cashD = getCashDollarsInput();
	const nav = sumPositionMarketValues() + cashD;
	...
	for (const card of document.querySelectorAll("#positions-body .position-row-wrap")) {
		...
		wEl.textContent = `${formatNumber(pct, {
			minimumFractionDigits: 1,
			maximumFractionDigits: 1
		})}%`;
	...
	if (totEl) {
		...
		else if (Math.abs(target - 100) > .5) totEl.classList.add("warn");
	}
	refreshPositionsState();
}
...
function localizeAgentCards() {
	document.querySelectorAll(".agent-card").forEach((card) => {
		...
		if (statusEl) {
			const state = card.classList.contains("running") ? "running" : card.classList.contains("done") ? "done" : card.classList.contains("error") ? "error" : card.classList.contains("waiting") ? "waiting" : "idle";
...
function applyLocaleToLiveUi() {
	...
	document.querySelectorAll("button").forEach((btn) => {
		if (!btn.disabled || btn.getAttribute("aria-busy") !== "true") btn.dataset.labelIdle = btn.textContent.trim();
	});
	localizeAgentCards();
	...
	setDataLoaderStatus(currentDataLoaderUi.state, currentDataLoaderDetailInput(), currentDataLoaderUi.allowRetry, { recordHistory: false });
	renderDataLoaderHistory();
	initSavedReview();
	renderHistoryList();
	if (currentResultsView) applyResultsFromData(currentResultsView.manager, currentResultsView.validation, currentResultsView.bundle || currentResultsView.agentOutputs);
	if (_drawerOpen && _drawerAgent) renderDrawer(_drawerAgent);
	loadModelConfigUi();
}
document.addEventListener("DOMContentLoaded", async () => {
	...
	document.getElementById("load-track-record")?.addEventListener("click", loadTrackRecord);
...
function parsePortfolioCsv(text) {
	...
	if (headers.indexOf("ticker") === -1) throw new Error(t("review.csvMissingTicker"));
	...
	const positions = objects.filter((row) => row.ticker).map((row) => {
		const assetClass = row.asset_class || "equity";
		if (!POSITION_ASSET_CLASSES.includes(assetClass)) throw new Error(t("review.csvInvalidAssetClass", { assetClass }));
		return {
			ticker: row.ticker,
			name: row.name || "",
			sector: row.sector || "",
...
function portfolioCsvFromForm() {
	...
	return [PORTFOLIO_CSV_COLUMNS.join(","), ...rows.map((row) => PORTFOLIO_CSV_COLUMNS.map((column) => csvEscape(row[column])).join(","))].join("\n") + "\n";
}
function portfolioCsvFilename() {
	return `${(document.getElementById("p-name").value.trim() || "portfolio").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "") || "portfolio"}-${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}.csv`;
}
...
function buildPortfolio() {
	...
	const rows = Array.from(document.querySelectorAll("#positions-body .position-row-wrap"));
	const cashD = getCashDollarsInput();
	for (const wrap of rows) sumPos += getPositionMarketValue(wrap);
...
async function startReview() {
	...
	if (missingQuotes.length > 0) {
		...
		if (missingQuotes.length > 0) {
			showToast(t("review.marketDataRequired", { tickers: missingQuotes.join(", ") }), true, 9e3);
			return;
		}
...
async function runReviewWithBody(startBody, options = {}) {
	...
	hideResultsModal();
	setDataLoaderStatus("running", fromRetry ? dataLoaderDetail("dataLoader.retrying") : dataLoaderDetail("dataLoader.starting"), false);
	let res;
	try {
	...
	const review_id = (await res.json()).review_id;
	if (!review_id) {
...
async function stopReview() {
	...
	try {
		...
		showToast(t("review.stopReviewError", { detail: err instanceof Error ? err.message : String(err) }), true);
	...
	switch (msg.type) {
		...
		case "agent_done":
			...
			if (msg.agent === "manager") {
				...
				renderFinalResultOnce(pendingFinalResult);
				stopReviewElapsedTimer();
				setGlobalStatus("done");
				setButtonBusy(document.getElementById("start-btn"), false);
				setStopButtonRunning(false);
		...
		case "error":
			{
				...
				stopReviewElapsedTimer();
				const failureDetail = isDataLoaderError(msg.message) ? formatDataLoaderError(msg.message) : String(msg.message || t("dataLoader.reviewFailed"));
				setGlobalStatus("error", failureDetail);
				const failedAgent = msg.agent || _currentActiveAgent;
				if (failedAgent) setCardState(failedAgent, "error", failureDetail);
				if (failedAgent === "data") setAllDataLoaderTickers("error");
				setDataLoaderStatus("error", failureDetail, isDataLoaderError(msg.message));
			}
			setButtonBusy(document.getElementById("start-btn"), false);
			setStopButtonRunning(false);
			console.error("[review error]", msg.message);
			showToast(msg.message || t("dataLoader.reviewFailed"), true, 12e3);
			_currentActiveAgent = null;
			break;
		case "stopped":
			stopReviewElapsedTimer();
...
function setCardState(agent, state, detail = "") {
	...
	card.className = `agent-card ${state}${card.classList.contains("data-loader-panel") ? " data-loader-panel" : ""}${stepsOpen ? " steps-open" : ""}`;
	const label = card.querySelector(".agent-status-label");
	if (label) {
		const detailText = String(detail || "").trim();
		label.textContent = state === "error" && detailText ? compactStatusDetail(detailText) : AGENT_STATUS_LABELS[state]?.() ?? state;
...
function hasMeaningfulPortfolioVerdict(verdict) {
	...
	return [
		...
		"revisit_trigger",
		"rationale"
	].some((field) => String(verdict[field] || "").trim()) || normalizePriority(verdict.action_timing) !== "watch";
}
...
function normalizeActionScope(action) {
	const portfolioScopes = new Set([
		"portfolio",
		"portfolio-level",
		"portfolio_level",
		"book"
	]);
	const positionScopes = new Set([
		"position",
		"ticker",
		"security",
		"holding"
...
function appendEvidenceList(parent, evidence) {
	const items = Array.isArray(evidence) ? evidence.map((item) => String(item || "").trim()).filter(Boolean) : String(evidence || "").trim() ? [String(evidence).trim()] : [];
...
function evidenceItems(evidence) {
	return Array.isArray(evidence) ? evidence.map((item) => String(item || "").trim()).filter(Boolean) : String(evidence || "").trim() ? [String(evidence).trim()] : [];
}
...
function renderActionCards(actions) {
	...
	actions.forEach((action, index) => {
		...
		const setExpanded = (expanded) => {
			...
			top.setAttribute("aria-label", `${expanded ? t("results.hideActionDetails") : t("results.showActionDetails")}: ${title.textContent}`);
		};
	...
	switch (agent) {
		case "news": {
			...
			if (rawRes && String(rawRes).trim()) {
				...
				chunks.push(`<b>${escapeHtml(t("agentOutput.news.toolResearchExcerpt"))}</b> ${escapeHtml(s.slice(0, 480))}${s.length > 480 ? "…" : ""}`);
			}
...
function isManagerReviewLike(value) {
	...
	return MANAGER_REVIEW_FIELDS.some((field) => Object.prototype.hasOwnProperty.call(value, field));
}
...
function compactStepLogsForStorage(stepLogs) {
	...
	for (const [agent, logs] of Object.entries(stepLogs)) {
		...
		out[agent] = logs.slice(-MAX_PERSISTED_STEP_LOGS).map((entry) => ({
			at: String(entry?.at || ""),
...
function restoreAnalysisTraceFromBundle(bundle) {
	...
	if (bundle.agentOutputs && typeof bundle.agentOutputs === "object") Object.assign(_agentOutputs, bundle.agentOutputs);
	if (bundle.agentOutputUpdatedAt && typeof bundle.agentOutputUpdatedAt === "object") Object.assign(currentAgentOutputUpdatedAt, bundle.agentOutputUpdatedAt);
	if (bundle.stepLogs && typeof bundle.stepLogs === "object") for (const [agent, logs] of Object.entries(bundle.stepLogs)) {
		if (!Array.isArray(logs)) continue;
...
function sortedMetricEntries(values, { absolute = false } = {}) {
	...
	return Object.entries(values).map(([name, value]) => [name, toFiniteNumber(value)]).filter((entry) => entry[1] != null).sort((a, b) => {
...
function parseNewsResearchRuns(raw) {
	...
	return text.split(/\n\n---\n\n/g).map((block) => {
		...
		return match ? {
			...
			query: t("agentAnalysis.news.unknownQuery"),
			body: block.trim()
		};
...
function applyResultsFromData(manager, validation, bundleOrAgentOutputs = null) {
	...
	resetTrackRecordSection();
}
function resetTrackRecordSection() {
	const statusEl = document.getElementById("track-record-status");
	const bodyEl = document.getElementById("track-record-body");
	const btn = document.getElementById("load-track-record");
	if (statusEl) statusEl.textContent = "";
	if (bodyEl) bodyEl.innerHTML = "";
	if (btn) btn.disabled = false;
}
function formatDailyOutperf(value) {
	if (value === null || value === void 0 || Number.isNaN(Number(value))) return "—";
	const num = Number(value);
	return `${num >= 0 ? "+" : ""}${num.toFixed(2)}%`;
}
async function loadTrackRecord() {
	const reviewId = currentResultsView?.bundle?.reviewId || currentReviewId || "";
	const statusEl = document.getElementById("track-record-status");
	const bodyEl = document.getElementById("track-record-body");
	const btn = document.getElementById("load-track-record");
	if (bodyEl) bodyEl.innerHTML = "";
	if (!reviewId) {
		if (statusEl) statusEl.textContent = t("results.trackRecord.error");
		return;
	}
	if (btn) btn.disabled = true;
	if (statusEl) statusEl.textContent = t("results.trackRecord.loading");
	try {
		const res = await fetch(`/api/review/${encodeURIComponent(reviewId)}/track-record`);
...
function renderTrackRecord(data) {
	...
	if (!outcomes.length) {
		if (statusEl) statusEl.textContent = t("results.trackRecord.none");
		return;
	}
	if (statusEl) statusEl.textContent = t("results.trackRecord.summary", {
		validated: data.validated_count ?? 0,
		matured: data.matured_count ?? 0,
		benchmark,
	...
	const headers = [
		t("results.trackRecord.columns.position"),
		t("results.trackRecord.columns.call"),
		t("results.trackRecord.columns.days"),
		t("results.trackRecord.columns.daily", { benchmark }),
		t("results.trackRecord.columns.verdict")
	];
	...
	for (const outcome of outcomes) {
		...
		const positionEl = document.createElement("b");
		positionEl.textContent = outcome.position ?? "—";
		posTd.appendChild(positionEl);
		tr.appendChild(posTd);
		const callTd = document.createElement("td");
		const actionType = normalizeActionType(outcome.action_type);
		const callChip = document.createElement("span");
		callChip.className = `table-chip action-chip action-${actionType}`;
		callChip.textContent = actionTypeLabel(actionType);
		callTd.appendChild(callChip);
		tr.appendChild(callTd);
		const daysTd = document.createElement("td");
		daysTd.textContent = outcome.verdict === "pending" && !outcome.days_elapsed ? "—" : String(outcome.days_elapsed ?? 0);
		tr.appendChild(daysTd);
		const dailyTd = document.createElement("td");
		dailyTd.className = "track-record-daily";
		dailyTd.textContent = formatDailyOutperf(outcome.daily_outperf_pct);
		tr.appendChild(dailyTd);
		const verdictTd = document.createElement("td");
		const verdictChip = document.createElement("span");
		verdictChip.className = `table-chip track-verdict track-verdict--${outcome.verdict}`;
		verdictChip.textContent = t(`results.trackRecord.verdicts.${outcome.verdict}`);
		if (outcome.note) verdictChip.title = outcome.note;
		verdictTd.appendChild(verdictChip);
...
function hideHistoryModal() {
	...
	if (!el) return;
	el.classList.add("hidden");
	if (!document.getElementById("results-modal")?.classList.contains("hidden")) return;
	if (!document.getElementById("agent-analysis-modal")?.classList.contains("hidden")) return;
	if (!document.getElementById("llm-config-modal")?.classList.contains("hidden")) return;
	document.body.style.overflow = "";
}
	...
	try {
		...
		if (currentReviewId === reviewId) {
			...
			if (attempt < 3) setTimeout(() => {
				renderFinalResultOnce(finalResult, attempt + 1);
			}, 700 * (attempt + 1));
			else showToast(t("review.invalidResponseToast"), true);
		}
	} finally {
...
function recordStep(agent, stepIndex, label) {
	if (!agentStepProgress[agent]) agentStepProgress[agent] = {
		active: -1,
		done: /* @__PURE__ */ new Set(),
		labels: {}
	};
...
function completeAllSteps(agent) {
	...
	if (!state) return;
	const plan = getAgentPlan(agent);
	if (plan) for (let i = 0; i < plan.steps.length; i++) state.done.add(i);
	state.active = -1;
	if (document.getElementById(`card-${agent}`)?.classList.contains("steps-open")) renderAgentStepStatus(agent);
	else clearStreamLog(agent);
}
...
function togglePin() {
	...
	if (_drawerPinned) {
		...
		bar.textContent = t("drawer.pinnedTo", { agent: getAgentPlan(_drawerAgent)?.label || _drawerAgent });
...
function drawerAutoFollowDelayed(agent) {
	...
	_autoFollowTimer = setTimeout(() => {
		_autoFollowTimer = null;
		if (_currentActiveAgent && _currentActiveAgent !== _drawerAgent) switchDrawerTo(_currentActiveAgent);
	}, AUTO_FOLLOW_DELAY_MS);
}
function switchDrawerTo(agent) {
	if (_drawerAgent === agent) {
		renderDrawer(agent);
		return;
	}
...
function renderDrawer(agent) {
	...
	const card = document.getElementById(`card-${agent}`);
	const cardState = card?.classList.contains("running") ? "running" : card?.classList.contains("done") ? "done" : card?.classList.contains("error") ? "error" : card?.classList.contains("waiting") ? "waiting" : "idle";
	...
	if (start && end) timeEl.textContent = fmtDuration(Math.max(0, Math.round((end - start) / 1e3)));
	else if (start) timeEl.textContent = fmtDuration(Math.floor((Date.now() - start) / 1e3));
	...
	if (_agentOutputs[agent]) {
		...
		outputEl.innerHTML = `<div class="drawer-output-heading">${updatedAt ? `${escapeHtml(t("drawer.output"))} · ${escapeHtml(formatSavedAt(updatedAt))}` : escapeHtml(t("drawer.output"))}</div><pre>${drawerRawMode ? escapeHtml(JSON.stringify(_agentOutputs[agent], null, 2)) : agentOutputHtml(agent, _agentOutputs[agent])}</pre>`;
...
function renderAgentSummaryPopup() {
	...
	if (bulletsEl) {
		bulletsEl.innerHTML = item.bullets.map((bullet) => `<li>${escapeHtml(bullet)}</li>`).join("");
		bulletsEl.classList.toggle("hidden", item.bullets.length === 0);
	}
	if (dismissEl) dismissEl.textContent = agentSummaryQueue.length > 0 ? t("agentSummary.next") : t("agentSummary.dismiss");
...
function dismissAgentSummary() {
	...
	if (dismissed?.agent === "manager" && pendingFinalResult) renderFinalResultOnce(pendingFinalResult);
	showNextAgentSummary();
}
...
function sendCompletionNotification() {
	...
	const elapsed = _reviewStartTime ? fmtDuration(Math.round((Date.now() - _reviewStartTime) / 1e3)) : "";
	const body = elapsed ? `${t("notifications.completedIn", { elapsed })}` : t("notifications.completed");
...
function setGlobalStatus(state, detail = "") {
	...
	const detailText = String(detail || "").trim();
	el.textContent = state === "error" && detailText ? t("status.failedWithReason", { reason: compactStatusDetail(detailText) }) : labels[state] || state;
	el.title = detailText;
}
function resetCards() {
	...
	document.querySelectorAll(".agent-card").forEach((card) => {
		const agent = card.dataset.agent;
		card.className = `agent-card${card.classList.contains("data-loader-panel") ? " data-loader-panel" : ""}`;
		if (agent !== "data") wireAgentCardInteractions(card, agent);
		const label = card.querySelector(".agent-status-label");
	...
	if (followBar) {
		followBar.textContent = t("drawer.autoFollowing");
		followBar.classList.remove("pinned-bar");
	}
...
//#endregion
//#region frontend/advisor/src/layout-mirror.js
var AGENTS = [
	"data",
	"planner",
	"news",
...
//#endregion
//#region frontend/advisor/src/main.js
